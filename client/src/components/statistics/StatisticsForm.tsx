...
export default function StatisticsForm({ gameId, players, rosters, gameStats }: StatisticsFormProps) {
  ...
  const deleteStatsMutation = useMutation({
    mutationFn: async (statId: number) => {
      return apiClient.delete(`/api/games/${gameId}/stats/${statId}`);
    },
    onSuccess: () => {
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      for (const playerIdStr in players) {
        ...
        if (existingStat) {
          // Update existing
          promises.push(apiClient.patch(`/api/games/${gameId}/stats/${existingStat.id}`, statsData));
        } else {
          // Create new
          promises.push(apiClient.post(`/api/games/${gameId}/stats`, statsData));
        }
      }
