...
export function OfficialScoreEntry({
  ...
  useEffect(() => {
    const loadScores = async () => {
      try {
        const existingScores = await apiClient.get(`/api/games/${gameId}/scores`);
        if (existingScores) {
  ...
  // Calculate totals whenever quarter scores change
  useEffect(() => {
    const homeTotal = scores.homeTeamQ1 + scores.homeTeamQ2 + scores.homeTeamQ3 + scores.homeTeamQ4;
    const awayTotal = scores.awayTeamQ1 + scores.awayTeamQ2 + scores.awayTeamQ3 + scores.awayTeamQ4;
  ...
  }, [scores.homeTeamQ1, scores.homeTeamQ2, scores.homeTeamQ3, scores.homeTeamQ4,
      scores.awayTeamQ1, scores.awayTeamQ2, scores.awayTeamQ3, scores.awayTeamQ4]);

  const handleScoreChange = (field: keyof GameScores, value: string) => {
    const numValue = parseInt(value) || 0;
    setScores(prev => ({
      ...prev,
      [field]: numValue
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
      await apiClient.post(`/api/games/${gameId}/scores`, scores);
      toast({
        title: "Success",
      ...
      toast({
        title: "Error",
        description: "Failed to save official scores",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          Official Game Scores
          {hasExistingScores && <Badge variant="secondary">Saved</Badge>}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[1, 2, 3, 4].map(quarter => (
                <div key={quarter}>
                  <Label htmlFor={`home-q${quarter}`}>Q{quarter}</Label>
                  <Input
                    id={`home-q${quarter}`}
                    type="number"
                    min="0"
                    value={scores[`homeTeamQ${quarter}` as keyof GameScores]}
                    onChange={(e) => handleScoreChange(`homeTeamQ${quarter}` as keyof GameScores, e.target.value)}
                  />
                </div>
              ))}
            </div>
            <div className="text-xl font-bold">
              Total: {scores.homeTeamTotal}
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[1, 2, 3, 4].map(quarter => (
                <div key={quarter}>
                  <Label htmlFor={`away-q${quarter}`}>Q{quarter}</Label>
                  <Input
                    id={`away-q${quarter}`}
                    type="number"
                    min="0"
                    value={scores[`awayTeamQ${quarter}` as keyof GameScores]}
                    onChange={(e) => handleScoreChange(`awayTeamQ${quarter}` as keyof GameScores, e.target.value)}
                  />
                </div>
              ))}
            </div>
            <div className="text-xl font-bold">
              Total: {scores.awayTeamTotal}
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <Textarea
            id="notes"
            placeholder="Any additional notes about the game scores..."
            value={scores.notes || ''}
        ...
        <Button 
          ...
          className="w-full"
        >
          {loading ? 'Saving...' : hasExistingScores ? 'Update Official Scores' : 'Save Official Scores'}
        </Button>
      </CardContent>
    </Card>
  );
