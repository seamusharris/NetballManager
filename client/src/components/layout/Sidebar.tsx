...
export default function Sidebar({ isMobileOpen, setIsMobileOpen, isTablet }: SidebarProps) {
  ...
  const getNavLinks = () => {
    const clubWideLinks = [
      ...
      { path: '/team-ladder', label: 'Team Ladder', icon: <Trophy className="w-5 h-5" />, section: 'club' },
    ];
    ...
    const devLinks = [
      { path: '/component-examples', label: 'Component Examples', icon: <Zap className="w-5 h-5" />, section: 'dev' },
    ...
    const teamLinks = currentTeamId ? [
      ...
      { 
        path: nextGame ? `/team/${currentTeamId}/preparation/${nextGame.id}/dashboard` : `/team/${currentTeamId}/preparation`,
