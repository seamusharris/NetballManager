    ...
    mutationFn: async ({ 
      ...
      if (existingRosters.length > 1) {
        ...
        for (let i = 1; i < existingRosters.length; i++) {
          await apiClient.delete(`/api/rosters/${existingRosters[i].id}`);
        }

        // Update the newest entry
        const res = await apiClient.patch(`/api/rosters/${existingRosters[0].id}`, {
          playerId
        });
        return res.json();
      }
      ...
      // No existing entries, create a new one
        ...
        const res = await apiClient.post('/api/rosters', {
  ...
  const handleAutoFill = async () => {
    ...
    try {
      ...
      for (const assignment of assignments) {
        const savePromise = apiClient.post('/api/rosters', assignment).then(res => res.json());
    ...
    try {
      ...
      for (const roster of rosters) {
        if (roster.gameId === selectedGameId && roster.quarter === parseInt(activeQuarter)) {
          toDeletePromises.push(apiClient.delete(`/api/rosters/${roster.id}`, {}));
        }
      }
      ...
      Object.entries(sourceQuarterAssignments).forEach(([position, playerId]) => {
        if (playerId !== null) {
          ...
          const savePromise = apiClient.post('/api/rosters', {
            gameId: selectedGameId,
