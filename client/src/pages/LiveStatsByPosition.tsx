...
import { GameStat, Position, allPositions, Roster, Game } from '@shared/schema';
...
export default function LiveStatsByPosition() {
  ...
  const { data: opponent, isLoading: isLoadingOpponent } = useQuery<any>({
  ...
  const updateGameStat = async (id: number, data: any): Promise<boolean> => {
    try {
      ...
      const response = await apiClient.patch(`/api/game-stats/${id}`, data);
