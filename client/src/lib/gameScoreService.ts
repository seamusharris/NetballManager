...
class GameScoreService {
  async calculateGameScores(gameStats: any[], gameStatus?: string, statusScores?: { teamGoals: number | null; opponentGoals: number | null }, isInterClub: boolean = false, homeTeamId?: number, awayTeamId?: number, currentTeamId?: number, officialScores?: OfficialGameScore[], gameId?: number): Promise<GameScores> {
  ...
  private createFixedScores(teamGoals: number, opponentGoals: number): GameScores {
    ...
    const result = teamGoals > opponentGoals ? "win" : teamGoals < opponentGoals ? "loss" : "draw";
  ...
  private convertLegacyScores(legacy: any): GameScores {
    ...
    return {
      ...
      result: legacy.finalScore.for > legacy.finalScore.against ? "win" : legacy.finalScore.for < legacy.finalScore.against ? "loss" : "draw",
    };
  }
  ...
  private createScoresFromOfficial(officialScores: OfficialGameScore[], homeTeamId?: number, awayTeamId?: number, currentTeamId?: number): GameScores {
    ...
    let result: "win" | "loss" | "draw" = homeTotal > awayTotal ? "win" : homeTotal < awayTotal ? "loss" : "draw";
   ...
   */
  calculateGameScoresSync(gameStats: any[], gameStatus?: string, statusScores?: { teamGoals: number | null; opponentGoals: number | null }, isInterClub: boolean = false, homeTeamId?: number, awayTeamId?: number, currentTeamId?: number, officialScores?: OfficialGameScore[]): GameScores {
    // Use provided official scores if available
    if (officialScores && officialScores.length > 0) {
      return this.createScoresFromOfficial(officialScores, homeTeamId, awayTeamId, currentTeamId);
    }
